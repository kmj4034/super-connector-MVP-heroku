...
// models 객체를 models/index에서 불러와서, models.Users로 접근하면 오류 해결됨.
...
//   const { email, password, nickname } = req.body; // 프론트에서 보낸 폼데이터를 받는다.
//   try {
...
router.get("/kakao", passport.authenticate("kakao")); // 로그인 화면 띄우기. passport.use 호출 -> 로그인하면 미리 설정한 redirectURI로 리다이렉트됨 -> passport.authenticate 호출 -> 카카오 서버에서 인증정보를 받아서 인증에 필요한 정보를 전달하면서 인증완료된다.
router.get(
  "/kakao/callback",
  passport.authenticate("kakao", {
    // 사용자가 동의하면 passport.authenticate("kakao")에서 kakaoStrategy로 인증 시행
